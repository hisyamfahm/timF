...
?>
						 ...
						 <a class="text-dark" href="openpdf_berita.php?id=<?php echo $row_query['Id_berita']; ?>">di sini</a><br>
...
<?php 
		include 'koneksi.php';
		$data = mysqli_query($koneksi,"select * from berita order by Id_berita desc limit 4");
		while($d = mysqli_fetch_array($data)){
?>
                        <div class="col">
                        <img src="foto/<?php echo $d ['Foto']?>" width="35%" style="float:left;" class="img-fluid">
						<p>
                        <?php echo $d['Judul']; ?>
						<br>
						<span><?php echo $d['Tanggal']; ?></span>
						<br>
                         <a href="berita_show.php?id=<?php echo $d['Id_berita']; ?>">Read More</a></p> 
                       </div>
  <?php 
		}
		?>					   
 ...
 <?php 
		include 'koneksi.php';
		$data = mysqli_query($koneksi,"select * from pengumuman order by Id_pengumuman desc limit 6");
		while($d = mysqli_fetch_array($data)){
			?>
            <div class="pengumuman pt-2">
			<a href="pengumuman_show.php?id=<?php echo $d['Id_pengumuman']; ?>" class="text-dark"><?php echo $d['Judul']; ?></a>
			</div>
  <?php 
		}
		?>
...
<div>
		...
		while($d = mysqli_fetch_array($data)){
			?>
                       <div class="pengumuman pt-2">
					   <a href="agenda.php" class="text-dark"><?php echo $d['Tanggal']; ?></a>
					   </div>
  <?php 
		}
		?>
