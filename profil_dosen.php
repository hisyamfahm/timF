...
$query = mysqli_query($koneksi,"SELECT * FROM dosen inner join users on users.Username=dosen.NIP WHERE NIP = '$NIP'");
...
?>
    ...
    <div class="jumbotron jumbotron-fluid bg-dark text-white pb-4">
	  ...
	  <img src="foto/<?php echo $row_query ['Foto']?>" width="200" class="rounded-circle  img-thumbnail">
        <h3 class="display-5 mt-4"><?php echo $row_query ['Nama_dosen']?>
        </h3>
      ...
      <div class="container pb-4">
		...
		<form class="login100-form validate-form" method="post" action="" enctype = "multipart/form-data">
		  ...
		  <div class="form-group">
            ...
            <input type="text" name="nama" class="form-control" id="exampleInputPassword1" value="<?php echo $row_query ['Nama_dosen']?>">
          </div>
		  <div class="form-group">
            <label for="exampleInputPassword1">Status Jabatan</label>
            <input type="text" name="jabatan" class="form-control" id="exampleInputPassword1" value="<?php echo $row_query ['Status_jabatan']?>">
          </div>
		  <div class="form-group">
            <label for="exampleInputPassword1">Bidang Kajian</label>
            <input type="text" name="jabatan" class="form-control" id="exampleInputPassword1" value="<?php echo $row_query ['Bidang_kajian']?>">
          </div>
		  <div class="form-group">
...
$NIP = $_SESSION['Username'];
$sql = "select * FROM dosen inner join users on users.Username=dosen.NIP WHERE NIP = '$NIP'";
$query = mysqli_query($koneksi, $sql);
$count = mysqli_num_rows($query);

if($count == 0) {
$sql= "INSERT INTO dosen (NIP, Nama_dosen, Status_jabatan, Jenis_kelamin, Tempat_lahir, Tanggal_lahir, Alamat, No_hp, Foto, Bidang_kajian) 
VALUES ('$NIP','$nama','$jabatan','$kelamin','$tempat','$tanggal','$alamat','$phone','$name','$bidang')" ;
}else{
$sql="UPDATE dosen SET Nama_dosen='$nama', Status_jabatan='$jabatan', Jenis_kelamin='$kelamin', Tempat_lahir='$tempat', 
Tanggal_lahir='$tanggal', Alamat='$alamat', No_hp='$phone', Foto='$name', Bidang_kajian='$bidang' WHERE NIP = '$NIP'";
}
